...
>> In this view, the data is a random variable, while the probability is fixed - from the idea that when N is very very large, n is the total number of the specific outcome - and the variability comes from the sampling this data.
...
>> Where we want to determine the probability of A given that B has occurred. This can be calculated if we know what is the probability of B given A has occurred along with the probabilities of A and B separately and individually. 

>> The idea of Bayesian probability makes sense if we were to calculate the probability of events that cannot be repeated infinitely.
